---
last_mapped: 2026-05-21T13:00:00Z
...
> Auto-generated by Cartographer. Last mapped: 2026-05-21.
...
- **Unvalidated `req.body` casts**: `testnets.ts:21,124,144,167,185,202`, `debugger.ts:141`, `source.ts:86,171` use TypeScript `as` casts instead of Zod schemas. Type errors leak into service layer.
...
### Minor (still present)
- **`StatusBadge` duplicated 3×** — three independent implementations:
  - `components/SimulationResult.tsx:9` — boolean prop, `w-2 h-2` dot
  - `components/BundleSimulator/StatusBadge.tsx:1` — boolean prop, `w-1.5 h-1.5` dot
  - `components/explorer/TxDetail/primitives.tsx:7` — string enum prop, larger padding
- **`getOpcodeColor` duplicated 2×** (was 3× in old map — one copy consolidated by the StepDebugger split):
  - `components/debugger/GasProfiler/colors.ts:30` — simpler, 4 categories, falls back to `var(--color-text-primary)`
  - `components/debugger/StepDebugger/theme.ts:26` — full table with `PUSH*`/`DUP*`/`SWAP*`/`LOG*` prefix matching, falls back to `"#94A3B8"`. Different palette (SSTORE `#f97316` vs `#F59E0B`). A comment in `theme.ts:3-6` flags this for a future theming refactor.
  - The SDK also exports its own `getOpcodeColor` from `components/index.ts` (different palette again).
...
- **`useSourceMappings` cache key uses `pcs.length` not content**: `useContractSource.ts:16` — same number of PCs from a different call frame returns stale mappings.
...
- **[fixed 2026-05-21]** StepDebugger `getOpcodeColor` duplication centralized into `StepDebugger/theme.ts` (was previously inlined in `StepDebugger.tsx`).
