 ...
 * setCachedTrace(...)` call sites below) — that's intentional to avoid
 * adding DB latency to the response. The tradeoff is that a SIGTERM
 * during a write would drop the cache entry; this Set lets the shutdown
 * handler call `awaitPendingCacheWrites()` to drain pending writes
 * before exit.
 */
